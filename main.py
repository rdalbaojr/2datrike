...
from fastapi import FastAPI, Depimport os
import shutil
import json
from datetime import datetime
...
def get_profile(display_name: str, db: Session = Depends(get_db)):
    ...
    # 🟢 FIX: STRICTLY prioritize fetching the Driver profile first to bypass Passenger clones
    ...
    # If no driver is found, then fall back to passenger profile
    if not user:
        user = db.query(User).filter(User.full_name == clean_name, User.role == 'passenger').first()
    if not user:
        user = db.query(User).filter(User.username == clean_name, User.role == 'passenger').first()
...
def update_profile(data: ProfileUpdateSchema, db: Session = Depends(get_db)):
    ...
    # 🟢 FIX: Ensure profile updates always target the correct role
    user = db.query(User).filter(User.full_name == clean_name, User.role == 'driver').first()
    if not user:
        user = db.query(User).filter(User.username == clean_name, User.role == 'driver').first()
    if not user:
...
def rate_ride(ride_id: int, data: RateRideSchema, db: Session = Depends(get_db)):
    ...
    if ride.driver_name:
        # 🟢 FIX: Ensure we rate the driver, not a passenger with the same name
        driver = db.query(User).filter(User.full_name == ride.driver_name, User.role == 'driver').first()
        if not driver:
            driver = db.query(User).filter(User.username == ride.driver_name, User.role == 'driver').first()
...
def get_payout_summary(db: Session = Depends(get_db)):
    ...
    for ride in unsettled_rides:
        ...
        if clean_driver_name not in payouts:
            # 🟢 FIX: Strictly grab the DRIVER'S bank details, never the passenger clone
            driver_user = db.query(User).filter(User.full_name == clean_driver_name, User.role == 'driver').first()
            if not driver_user:
                driver_user = db.query(User).filter(User.username == clean_driver_name, User.role == 'driver').first()
...
def get_pending_rides(db: Session = Depends(get_db)):
    ...
    for r in rides:
        # Get Passenger Phone
        pass_phone = "0"
        # 🟢 FIX: Strictly map Passenger to Passenger Role
        pass_user = db.query(User).filter(User.full_name == sanitize_name(r.passenger_name), User.role == 'passenger').first()
        if not pass_user:
            pass_user = db.query(User).filter(User.username == sanitize_name(r.passenger_name), User.role == 'passenger').first()
        ...
        drv_phone = "0"
        if r.driver_name:
            # 🟢 FIX: Strictly map Driver to Driver Role
            drv_user = db.query(User).filter(User.full_name == sanitize_name(r.driver_name), User.role == 'driver').first()
            if not drv_user:
                drv_user = db.query(User).filter(User.username == sanitize_name(r.driver_name), User.role == 'driver').first()
...
# ==========================================
app.mount("/", StaticFiles(directory="web", html=True), name="web")ends, Form, HTTPException, File, UploadFile, WebSocket, WebSocketDisconnect
from fastapi.staticfiles import StaticFiles
from fastapi.responses import RedirectResponse, JSONResponse
from pydantic import BaseModel
from sqlalchemy import create_engine, Column, Integer, String, Text, DateTime, Float, text
